...
const deepCleanSubjects = async () => {
    try {
        await mongoose.connect(process.env.MONGO_URI);
        console.log('Connected for Deep Clean...');

        const currentSubjects = await Subject.find({ course: 'B.Tech' });
        const keptIds = [];
        ...
        for (const target of targetList) {
            ...
            if (subject) {
                console.log(`[KEEPING] ${target.name}`);
        ...
        console.log(`Deep Clean complete! Deleted ${deleteRes.deletedCount} redundant subjects.`);
        process.exit(0);
    } catch (err) {
        console.error(err);
        process.exit(1);
